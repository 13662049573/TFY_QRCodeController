...
-(id)initWithFrame:(CGRect)frame style:(NSString *)style
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _scanType = TFY_ScanTypeQrCode;
        self.heightScale = 1;
        self.lineImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"scan_blue_line" inBundle:[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"TFY_resource" ofType:@"bundle"]] compatibleWithTraitCollection:nil]];
...
- (void)layoutSubviews {
    ...
    CGFloat YMinRetangle = self.frame.size.height / 3.0 - sizeRetangle.height/(2.0 * _heightScale);
...
- (void)startAnimating {
    ...
    CGFloat YMinRetangle = self.frame.size.height / 3.0 - sizeRetangle.height/2.0;
...
- (void)drawScanRect {
    ...
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithRed:1.00 green:0.31 blue:0.17 alpha:1.00].CGColor);
